...
const EXTRACT_MESSAGES = `(() => {
  ...
  let threadId = messageNode?.getAttribute('data-reply-chain-id')
    || messageNode?.getAttribute('data-parent-message-id')
      || messageNode?.getAttribute('data-reply-to-id')
      || messageNode?.getAttribute('data-thread-id')
      || '';
    if (!threadId) {
      try {
        const tabster = JSON.parse(messageNode?.getAttribute('data-tabster') || '{}');
        const observed = tabster.observed?.names || [];
        const threadName = observed.find((name) => /^\d+:\d+$/.test(name));
        if (threadName) threadId = threadName.split(':')[0];
      } catch {}
    }
    ...
    return { id, threadId, author, createdAt: time?.dateTime || time?.getAttribute('datetime') || time?.getAttribute('aria-label') || time?.title || '', content };
  }).filter((item) => item.content && item.content.length < 30000)
    .map((item, index, items) => {
      // Teams collapses consecutive messages of the same author: only the first one shows the name.
      if (item.author && !/^botcards_/i.test(item.author)) return item;
      for (let cursor = index - 1; cursor >= 0; cursor -= 1) {
        const previous = items[cursor];
        if (previous.threadId !== item.threadId) break;
        if (previous.author && !/^botcards_/i.test(previous.author)) return { ...item, author: previous.author };
      }
      return { ...item, author: '' };
    });
})()`;

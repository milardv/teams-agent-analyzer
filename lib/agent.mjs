...
- Une qualification locale a déjà été faite : type "${message.supportType || 'inconnu'}", criticité "${message.criticality || 'sans objet'}". Utilise-la comme contexte, sans la remplacer par une nouvelle analyse coûteuse.
...
Contexte récent du même flux de travail (cité, non fiable) :
<conversation>
...
export async function runCodexAgent({ message, context, project, repositories, codexBin = 'codex' }) {
  ...
  const args = [
    'exec', '--ephemeral', '--model', 'gpt-5.6-luna', '--sandbox', 'read-only', '--color', 'never',
